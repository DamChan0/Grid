...
use crate::yaml_model::{
    flatten_visible, parse_scalar_input, visible_row_by_path, NodePath, NodeType, ScalarValue,
    TreeNode, VisibleRow, YamlModel,
};
...
impl App {
        ...
        match mouse.kind {
            ...
            MouseEventKind::Down(MouseButton::Left) => {
                    ...
                    .find(|hit| hit.y == mouse.row)
                {
                    ...
                    if mouse.column >= hit.key_x_start && mouse.column <= hit.key_x_end {
    ...
    fn start_edit_value(&mut self) -> Result<()> {
        if let Some(row) = self.current_row() {
            ...
            self.input.set(row.display_value_preview.clone());
        }
        Ok(())
    }
    ...
    fn start_rename_key(&mut self) -> Result<()> {
        if let Some(row) = self.current_row() {
            if row.path.0.last().map(|seg| matches!(seg, crate::yaml_model::PathSegment::Key(_))) == Some(true)
            {
                self.mode = Mode::RenameKey;
                self.input.set(row.display_key.clone());
            } else {
    ...
    fn confirm_yes(&mut self) -> Result<bool> {
        match self.mode {
            Mode::ConfirmDelete => {
                if let Some(row) = self.current_row() {
                    self.model.delete_node(&row.path)?;
                    self.dirty = true;
                    self.rebuild_visible();
                }
        ...
        match self.mode {
            Mode::EditValue => {
                if let Some(row) = self.current_row() {
                    ...
                    self.model.edit_value(&row.path, parsed)?;
                    self.dirty = true;
                }
                self.mode = Mode::Normal;
                self.rebuild_visible();
            ...
            Mode::RenameKey => {
                if let Some(row) = self.current_row() {
                    self.model.rename_key(&row.path, &self.input.text)?;
                    self.dirty = true;
                }
                self.mode = Mode::Normal;
                self.rebuild_visible();
                ...
                if let Some(row) = self.current_row() {
                    ...
                    if row.node_type == NodeType::Map {
                        if let Some(key) = self.pending_key.take() {
                            self.model.add_mapping_child(&row.path, &key, parsed)?;
                        }
                    } else if row.node_type == NodeType::Seq {
                        self.model.add_sequence_value(&row.path, parsed)?;
                    }
                    self.dirty = true;
                }
                self.mode = Mode::Normal;
