...
fn draw_help(frame: &mut Frame<'_>, app: &App, area: Rect) {
    let mut text = "j/k: move  gg/G: top/bottom  h/l: collapse/expand  Enter: toggle  e: edit  r: rename  a: add  d: delete  /: search  n/N: next/prev  Ctrl+s: save  q: quit".to_string();
...
fn draw_overlay(frame: &mut Frame<'_>, app: &App, area: Rect) {
    let message = match app.mode {
        Mode::ConfirmDelete => Some("Delete node? (y/n)"),
        Mode::ConfirmQuit => Some("Unsaved changes. Quit? (y/n)"),
        _ => None,
    };
